...
class Agent():
	...
	def save_checkpoint(self, episode, filename="checkpoint.pth.tar"):
		state = {"state_dict": self.Q_eval.state_dict(), "optimizer": self.Q_eval.optimizer.state_dict(), "epsilon": self.epsilon, "episode": episode}
	...
	def load_checkpoint(self, checkpoint):
		...
		self.loaded_episode = checkpoint["episode"]
...
class AI(Node):
	...
	load_model = export(bool, default=True)

	def _ready(self):
		self.agent = Agent(gamma=0.99, epsilon=1.0, batch_size=500,
			n_actions=3, eps_end=0.001, input_dims=[23], lr=0.001, eps_dec=0.0001)

		if self.load_model:
			self.agent.load_checkpoint(T.load("checkpoint.pth.tar"))
	...
	def save_checkpoint(self, episode, filename="checkpoint.pth.tar"):
		self.agent.save_checkpoint(episode, filename)


	def get_loaded_episode(self):
		return self.agent.loaded_episode
